...
int Output::GetCellStartY(const CellPosition& cellPos) const
{
	...
	int Y = (UI.ToolBarHeight + (UI.CellHeight * cellPos.VCell()));
...
//////////////////////////////////////////////////////////////////////////////////////////
//Done (gives an error as it assumes that GUIlib is not included
...
//Q-I used CMUgraphics function (DrawPolygon) //almost done
void Output::DrawCell(const CellPosition& cellPos, int cardNum) const
{
	//.[0](SX,SY)--------.[3](SX+Width,SY)
   // .[1](SX,SY+Height)--------.[4](SX+Width,SY+Heright)
  // Get the Cell Number (from 1 to NumVerticalCells*NumHorizontalCells) and the X & Y of its upper left corner
	...
	int cellStartY = GetCellStartY(cellPos);
	//
	//int Xplace[4] = { cellStartX , cellStartX , cellStartX + UI.CellWidth , cellStartX + UI.CellWidth };
	//int Yplace[4] = { cellStartY , cellStartY + UI.CellHeight , cellStartY  , cellStartY + UI.CellHeight };
	...
	pWind->DrawRectangle(cellStartX, cellStartY, UI.CellWidth + cellStartX, UI.CellHeight + cellStartY);
...
void Output::DrawPlayer(const CellPosition& cellPos, int playerNum, color playerColor) const
{
	...
	if (playerNum >= 0 && playerNum <= 3)
	{
		...
		// Calculate the Y coordinate of the center of the player's circle (based on playerNum)
		int x = GetCellStartX(cellPos) + UI.LadderXOffset + radius + 4; // UI.LadderXOffset is used to draw players' circles 
...
void Output::DrawSnake(const CellPosition& fromCell, const CellPosition& toCell) const
{
...

	{
		...
		int xArr[4] = { x12,x12 + xChange,x12 ,cellStartX };
