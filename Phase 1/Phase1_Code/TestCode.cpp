...
int main()
{
	...
	pOut->ClearStatusBar();
	string z = "You entered " + to_string(i) + " ,Click to continue";
	pOut->PrintMessage(z);
	...
	for (int i = 0; i < 5; i++)
	{
		...
		if (b)
		{
			z = "vCell= " + to_string(cp.VCell()) + " ,hCell= " + to_string(cp.HCell()) + " ,click to continue";
			pOut->PrintMessage(z);
			pIn->GetPointClicked(x, y);
		}
		else
	...
	pOut->ClearStatusBar();
	pOut->PrintMessage("You Entered: " + z + " ,Click to continue");
	pIn->GetPointClicked(x, y);
	...
	for (int i = 0; i < 5; i++)
	{
		pOut->PrintMessage("click to Enter vCell");
		pIn->GetPointClicked(x, y);
		i = pIn->GetInteger(pOut);
		b = cellpos_1.SetVCell(i);
		if (b)
		{
			i = cellpos_1.VCell();
			pOut->PrintMessage("Now the vCell =" + to_string(i) + " ,clickto continue ");
			pIn->GetPointClicked(x, y);
		}
		else
		{
			pOut->PrintMessage("invalid vCell");
			pIn->GetPointClicked(x, y);
		}
	...
	for (int i = 0; i < 5; i++)
	{
		pOut->PrintMessage(" click Enter an hCell");
		pIn->GetPointClicked(x, y);
		i = pIn->GetInteger(pOut);
		b = cellpos_1.SetHCell(i);
		if (b)
		{
			i = cellpos_1.HCell();
			pOut->PrintMessage("Now the hCell =" + to_string(i) + " , click to continue");
			pIn->GetPointClicked(x, y);
		}
		else
		{
			pOut->PrintMessage("invalid hCell");
			pIn->GetPointClicked(x, y);
		}
	}
	...
	for (int i = 0; i < 5; i++)
	{
		...
		pOut->PrintMessage("click to Enter hCell");
		pIn->GetPointClicked(x, y);
		int h = pIn->GetInteger(pOut);
		CellPosition cp2(v, h);
		int num;
		num = cp2.GetCellNum();
		if (num == -1)
		{
		...
		else
		{
			...
			z = "Cell Number" + to_string(num) + " ,click to continue";
			pOut->PrintMessage(z);
			pIn->GetPointClicked(x, y);
		}
	...
	for (int i = 0; i < 5; i++)
	{
		pOut->PrintMessage(" click to Enter cellNum");
		pIn->GetPointClicked(x, y);
		i = pIn->GetInteger(pOut);
		CellPosition cp3(i);
		if (cp3.IsValidCell())
		{
			z = "vCell= " + to_string(cp3.VCell()) + ", hCell= " + to_string(cp3.HCell()) + ",click to continue";
			pOut->PrintMessage(z);
			pIn->GetPointClicked(x, y);
		}
		else
	...
	} while (ActType != EXIT_PLAY && ActType != EXIT_DESIGN);
